...
class ChatgptThread(QThread):
    ...
    def run(self):
        prompt = self.sendString
        loop = asyncio.new_event_loop()
        # Set
        asyncio.set_event_loop(loop)
        response = loop.run_until_complete(self.chat.ask(prompt))
        self.update_data.emit(response['answer'])


class MakeItTalkThread(QThread):
    video_update_data = pyqtSignal(str)
    ...
    def run(self):
        path_name = './examples/'
        # command = 'cd ../MakeItTalk-main & '
        command = 'python video.py --jpg ' + self.pic_name + '.jpg &'
        a = os.system(command)

        # video_path = Path('../MakeItTalk-main/examples')
        video_path = Path('./examples')
        video_name = self.pic_name + '_pred*'
        pic_fpaths = list(video_path.glob(video_name))
        print(pic_fpaths)
        ...
        command1 = 'ffmpeg -y -i ' + path_name + video_name + ' -filter:v "crop=256:256:256:256" '+ path_name + 'out.mp4'
        b = os.system(command1)
        self.video_update_data.emit('out.mp4')



class Toolbox:
    def __init__(self, enc_models_dir, syn_models_dir, voc_models_dir):
        self.radio_id = 0
        ...
        self.style_idx = 0
        self.ui.read_pic()
        self.reset_ui(enc_models_dir, syn_models_dir, voc_models_dir, None, None, None)
        self.setup_events()
        self.ui.start()
    ...
    def setup_events(self):
        ...
        # Chatgpt
        func = lambda: asyncio.run(self.ini_chatgpt())
        self.ui.send_chat_button.clicked.connect(func)

        # pic
        func = lambda: self.change_pic()
        self.ui.pic_box.currentIndexChanged.connect(func)

        # video
        func = lambda: self.btn_temp()
        self.ui.video_play_btn.clicked.connect(func)
        self.talk_thread = MakeItTalkThread()
        self.talk_thread.video_update_data.connect(self.play_video)
        self.ui.mediaPlayer.mediaStatusChanged.connect(self.change_video_state)
        self.ui.get_video_btn.clicked.connect(self.get_video)

        func = lambda: self.change_radio()
        self.ui.radio_button_group.buttonClicked.connect(func)
    ...
    def get_video(self):
        self.change_chat_text('', True)

    def change_video_state(self):
        if self.ui.mediaPlayer.state() == 0:
            out_path = Path('./examples/out.mp4')
            out1_path = Path('./examples/out1.mp4')
            copyfile(out_path, out1_path)
            url = '.\\examples\\out1.mp4'
            content = QMediaContent(QUrl.fromLocalFile(url))
            self.ui.mediaPlayer.setMedia(content)
            self.ui.video_play_btn.setDisabled(False)
            self.ui.video_play_btn.setText('播放上次合成视频')

    def change_radio(self):
        self.radio_id = self.ui.radio_button_group.checkedId()

    def change_pic(self):
        self.ui.pic_name = self.ui.pic_box.currentText()
        pixmap = QPixmap('.\\examples\\' + self.ui.pic_name + '.jpg')
        self.ui.pic_label.setPixmap(pixmap)

    def btn_temp(self):
        self.play_video('out.mp4')

    def play_video(self, data):
        self.ui.pic_layout_inner.replaceWidget(self.ui.pic_label, self.ui.videoWidget)
        url = '.\\examples\\' + data
        content = QMediaContent(QUrl.fromLocalFile(url))
        self.ui.send_chat_button.setDisabled(False)
        self.ui.mediaPlayer.setMedia(content)
        self.ui.mediaPlayer.play()
        self.ui.video_play_btn.setDisabled(False)

    async def ini_chatgpt(self):
        if self.send_ready:
            self.deal_chatgpt()
        else:
            self.ui.rcv_chat_text.setPlainText('')
            cf = ConfigParser()
            cf.read('cfg.ini')
            self.session_token = cf.get('chatgpt', 'session_token')
            if self.session_token is None:
                self.ui.rcv_chat_text.setPlainText('请在cfg.ini中配置session_token')
            else:
                try:
                    self.chat = Chatbot(self.session_token, "https://gpt.pawan.krd")
                    await asyncio.gather(self.chat.wait_for_ready())
                    self.ui.rcv_chat_text.setPlainText('Chatgpt连接成功')
                    self.ui.send_chat_button.setText('发送')
                    self.send_ready = True
                    self.chatgpt_thread = ChatgptThread(self.chat)
                    self.chatgpt_thread.update_data.connect(self.change_chat_text)
                except Exception as r:
                    self.ui.rcv_chat_text.setPlainText(r)

    def deal_chatgpt(self):
        prompt = self.ui.send_chat_text.toPlainText()
        self.ui.rcv_chat_text.appendPlainText('你： ' + prompt)
        self.chatgpt_thread.get_string(prompt)
        self.ui.send_chat_button.setDisabled(True)
        self.chatgpt_thread.start()
        self.ui.rcv_chat_text.appendPlainText('')
        self.ui.send_chat_text.setPlainText('')

    def change_chat_text(self, data, flag=False):
        if not flag:
            self.ui.text_prompt.setPlainText(data)
            self.ui.rcv_chat_text.appendPlainText('回答: ' + data)
            self.ui.rcv_chat_text.appendPlainText('')
            if self.radio_id == 0:
                self.ui.send_chat_button.setDisabled(False)
            elif self.radio_id == 1:
                self.synthesize()
                self.vocode()
            else:
                self.ui.pic_box.setDisabled(True)
                self.synthesize()
                self.vocode(True)
                self.ui.save_audio_file_default(self.current_wav, Synthesizer.sample_rate)
                self.ui.video_play_btn.setDisabled(True)
                self.ui.video_play_btn.setText('视频处理中')
                self.talk_thread.set_name(self.ui.pic_name)
    ...
    def reset_ui(self, encoder_models_dir, synthesizer_models_dir, vocoder_models_dir, extractor_models_dir,
                 convertor_models_dir, seed):
        self.ui.populate_browser(self.datasets_root, recognized_datasets, 0, True)
        self.ui.populate_models(encoder_models_dir, synthesizer_models_dir, vocoder_models_dir, extractor_models_dir,
                                convertor_models_dir, self.vc_mode)
    ...
    def synthesize(self):
        ...
        specs = self.synthesizer.synthesize_spectrograms(texts, embeds, style_idx=int(self.ui.style_slider.value()),
                                                         min_stop_token=min_token,
                                                         steps=int(self.ui.length_slider.value()) * 200)
