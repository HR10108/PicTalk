...
class UI(QDialog):
    ...
    def save_audio_file_default(self, wav, sample_rate):
        wav_path = Path('./examples/out.wav')
    ...
    def register_utterance(self, utterance: Utterance, vc_mode):
        ...
        if vc_mode:
            ...
            self.send_chat_button.setDisabled(False)
            self.synthesize_button.setDisabled(False)
            self.get_video_btn.setDisabled(False)
    ...
    def reset_interface(self, vc_mode):
        ...
        if vc_mode:
            ...
            self.send_chat_button.setDisabled(True)
            self.synthesize_button.setDisabled(True)
            self.get_video_btn.setDisabled(True)
    ...
    def read_pic(self):
        pic_path = Path('./examples')
        pic_fpaths = list(pic_path.glob("*.jpg"))
        self.pic_string_list = []
        for f in pic_fpaths:
            self.pic_string_list.append(f.stem)
        self.pic_cb_model.setStringList(self.pic_string_list)

    def __init__(self, vc_mode):
        ## Initialize the application
        self.app = QApplication(sys.argv)
        super().__init__(None)
        self.setWindowTitle("PicTalk")
        self.setWindowIcon(QtGui.QIcon('toolbox\\assets\\voice.png'))
        ...
        root_layout.addLayout(gen_layout, 2, 0)

        # Visualizations
        vis_layout = QVBoxLayout()
        root_layout.addLayout(vis_layout, 1, 0)

        # 聊天界面
        chat_layout = QGridLayout()
        root_layout.addLayout(chat_layout, 0, 1, 10, 10)

        # 图片和视频
        pic_layout = QGridLayout()
        root_layout.addLayout(pic_layout, 0, 12, 10, 22)
        ...
        source_groupbox = QGroupBox('克隆音源')
        ...
        self.browser_browse_button = QPushButton("打开本地")
        ...
        i += 2
        # Utterance box
        browser_layout.addWidget(QLabel("<b>当前音源:</b>"), i, 0)
        ...
        # Replay & Save Audio
        i = 0
        output_layout.addWidget(QLabel("<b>输出的音频:</b>"), i, 0)
        ...
        # 聊天界面设计
        chat_groupbox = QGroupBox('Chatgpt对话界面')
        chat_layout_inner = QVBoxLayout()
        chat_groupbox.setLayout(chat_layout_inner)
        chat_layout.addWidget(chat_groupbox, 0, 0)
        self.rcv_chat_text = QPlainTextEdit('请先调试左侧语音合成，然后再初始化Chatgpt')
        # self.rcv_chat_text.appendPlainText()
        self.rcv_chat_text.setMinimumWidth(500)
        self.rcv_chat_text.setReadOnly(True)
        chat_layout_inner.addWidget(self.rcv_chat_text, 5)
        self.send_chat_text = QPlainTextEdit('')
        chat_layout_inner.addWidget(self.send_chat_text, 1)
        self.send_chat_button = QPushButton("初始化")
        self.send_chat_button.setDisabled(True)
        chat_layout_inner.addWidget(self.send_chat_button)

        # 图片模块

        pic_groupbox = QGroupBox('MakeItTalk')
        pic_layout_inner = QVBoxLayout()
        self.pic_layout_inner = pic_layout_inner
        pic_groupbox.setLayout(pic_layout_inner)
        pic_layout.addWidget(pic_groupbox, 0, 0)
        self.pic_label = QLabel(self)
        self.pic_label.setFixedSize(512, 512)
        pixmap = QPixmap('.\\examples\\angelina.jpg')
        self.pic_name = 'angelina'
        self.pic_label.setScaledContents(True)
        self.pic_label.setPixmap(pixmap)
        pic_layout_inner.addWidget(self.pic_label)

        self.mediaPlayer = QMediaPlayer(None, QMediaPlayer.VideoSurface)
        self.videoWidget = QVideoWidget()
        self.videoWidget.setMinimumSize(512, 512)
        self.mediaPlayer.setVideoOutput(self.videoWidget)

        self.pic_box = QComboBox()
        self.pic_cb_model = QStringListModel()
        self.pic_box.setModel(self.pic_cb_model)
        pic_layout_inner.addWidget(self.pic_box)

        self.video_play_btn = QPushButton('播放上次合成视频')
        self.video_play_btn.setDisabled(True)
        pic_layout_inner.addWidget(self.video_play_btn)

        self.radio_button_chat = QRadioButton('仅对话')
        self.radio_button_voice = QRadioButton('对话与克隆声音')
        self.radio_button_video = QRadioButton('对话、克隆声音、生成播放视频')
        self.radio_button_group = QButtonGroup()
        self.radio_button_group.addButton(self.radio_button_chat, 0)
        self.radio_button_group.addButton(self.radio_button_voice, 1)
        self.radio_button_group.addButton(self.radio_button_video, 2)
        self.radio_button_chat.setChecked(True)
        pic_layout_inner.addWidget(self.radio_button_chat)
        pic_layout_inner.addWidget(self.radio_button_voice)
        pic_layout_inner.addWidget(self.radio_button_video)
        ...
        self.generate_button = QPushButton("合成并生成语音")
        gen_layout.addWidget(self.generate_button)
        layout = QHBoxLayout()
        self.synthesize_button = QPushButton("仅合成")
        layout.addWidget(self.synthesize_button)

        self.vocode_button = QPushButton("仅输出语音")
        layout.addWidget(self.vocode_button)
        gen_layout.addLayout(layout)

        self.get_video_btn = QPushButton("合成视频")
        gen_layout.addWidget(self.get_video_btn)
